...
public class MetronomePlayer implements Runnable {
	...
	private int instrument; // instrument number (uses midi percussion
	...
	// so if the rhythm is "1,5,12,14,20" it will sound at 0.25, 1.25, 3, 3.5 and 5
	// seconds
	// a bpm of 90 and resolution of 3 gives 4.5 ticks per second so would be only
	// slightly quicker
	...
	public void metronomeStop() {
		// stop the sequencer, only needed in case it exists and is running to prevent
		// unnecessary errors
			...
			while (isPlaying) {
				if (restart) {
					...
					if (rhythm.equals("")) {
						...
						for (long i = 1; i < (60 * resolution); i = i + resolution) {
						...
						for (String split : splits) {
							...
							track.add(new MidiEvent(off, tick + 1l));
						}
					}
					long totalTicks = track.ticks();
					long padTicks = totalTicks % resolution;
					if (padTicks > 0) {
						// add a silent note to the end so when it loops it does not sound two notes
						// close together
