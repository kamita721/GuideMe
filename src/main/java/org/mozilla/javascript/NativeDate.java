 ...
 *
 *         Modified to make class public 12/23/2020
 */
public final class NativeDate extends IdScriptableObject {
	...
	private static int YearFromTime(double t) {
		...
		/*
		 * Adjust the year if the approximation was wrong. Since the year was computed
		 * using the average number of ms per year, it will usually be wrong for dates
		 * within several hours of a year transition.
		 */
		if (t2 > t) {
			y--;
		} else {
	 ...
	 * determining DST; it hasn't been proven not to produce an incorrect year for
	 * times near year boundaries.
	 */
	...
	private static double date_parseString(String s) {
		...
		while (i < limit) {
			...
			if ('0' <= c && c <= '9') {
				...
				/*
				 * allow TZA before the year, so 'Wed Nov 05 21:49:11 GMT-0800 1997' works
				 */

				/*
				 * uses of seenplusminus allow : in TZA, so Java no-timezone style of GMT+4:30
				 * works
				 */
				...
				if (index < 2) {
					/*
					 * AM/PM. Count 12:30 AM as 00:30, 12:30 PM as 12:30, instead of blindly adding
					 * 12 if PM.
					 */
					if (hour > 12 || hour < 0) {
						return ScriptRuntime.NaN;
	...
	private static double makeTime(double date, Object[] args, int methodId) {
		if (args.length == 0) {
			/*
			 * Satisfy the ECMA rule that if a function is called with fewer arguments than
			 * the specified formal arguments, the remaining arguments are set to undefined.
			 * Seems like all the Date.setWhatever functions in ECMA are only varargs beyond
			 * the first argument; this should be set to undefined if it's not given. This
			 * means that "d = new Date(); d.setMilliseconds()" returns NaN. Blech.
			 */
			return ScriptRuntime.NaN;
		}
	...
	private static double makeDate(double date, Object[] args, int methodId) {
		...
		/*
		 * return NaN if date is NaN and we're not setting the year, If we are, use 0 as
		 * the time.
		 */
