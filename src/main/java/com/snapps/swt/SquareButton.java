 ...
 * Copyright 2009-2010 Strategic Network Applications, Inc.
 * (http://www.snapps.com).
 * <p>
 * 
...
public class SquareButton extends Canvas {
	...
	public static final int BG_IMAGE_CROP = 0;
	...
	protected void mouseExit(Event e) {
		if (isFocused)
			setSelectedColor(e);
		else
			setNormalColor(e);
	...
	protected void addListeners() {
		...
		// MOUSE EVENTS
		this.addListener(SWT.MouseEnter, this::setHoverColor);
		this.addListener(SWT.MouseExit, this::mouseExit);
		this.addListener(SWT.MouseUp, this::mouseUp);
		this.addListener(SWT.MouseHover, this::setHoverColor);
		this.addListener(SWT.MouseDown, this::setClickedColor);

		// TAB TRAVERSAL (a KeyDown listener is also required)
		this.addListener(SWT.Traverse, this::onTraverse);
		this.addListener(SWT.FocusIn, this::focusIn);
		this.addListener(SWT.FocusOut, this::focusOut);

		this.addListener(SWT.KeyUp, this::keyUp);
		this.addListener(SWT.KeyDown, this::keyDown);
		setTraversable(true);
	}
	...
	protected void focusOut(Event e) {
		isFocused = false;
	...
	/**
	 * This is an image that will be displayed to the side of the text inside the
	 * button (if any). By default the image will be to the left of the text;
	 * however, setImageStyle can be used to specify that it's either to the right
	 * or left. If there is no text, the image will be centered inside the button.
	 * 
	 * @param image
	 */
	 ...
	 * drawn in the manner specified by the backgroundImageStyle setting. The order
	 * in which the button is drawn is: background color, then background image,
	 ...
	 * <p>
	 * <ul>
	 * <li>BG_IMAGE_CROP: the image is drawn once, with the top left corner of the
	 * image at the top left corner of the button. Any part of the image that is too
	 * wide or too tall to fit inside the button area is clipped (cropped) off.</li>
	 * <li>BG_IMAGE_STRETCH: the image is stretched (or squashed) to exactly fit the
	 * button area.</li>
	 * <li>BG_IMAGE_TILE: the image is tiled vertically and horizontally to cover
	 * the entire button area.</li>
	 * <li>BG_IMAGE_CENTER: the center of the image is placed inside the center of
	 * the button. Any part of the image that is too tall or too wide to fit will be
	 * clipped.</li>
	...
	/**
	 * Set whether or not this button is enabled (active) or not (inactive). This
	 * setting can be changed dynamically after the button has been drawn.
	 * <p>
	 * An inactive button does not change color when it is hovered over or clicked,
	 * does not receive focus or participate in the tab order of the widget
	 * container, and does not notify listeners when clicked.
	 */
	@Override
	...
	/**
	 * Set the inner margin between the left and right of the text inside the button
	 * and the button borders, in pixels. Like the left and right padding for the
	 * text. Default is 8 pixels.
	 * 
	...
	/**
	 * Set the inner margin between the top and bottom of the text inside the button
	 * and the button borders, in pixels. Like the top and bottom padding for the
	 * text. Default is 4 pixels.
	 * 
	 ...
	 * selection border). If a selection border is used, it will be the same color
	 * as the font color. Note that you can also use setSelectedColors() to change
	 * the look of the button when it has focus.
	 * 
	 ...
	 * @param bgColor2 the gradient color at the bottom of the button (if you don't
	 *                 want a gradient, set this to bgColor1)
	 * @param bdrColor the color of the border around the button (if you don't want
	 *                 a border, use getBackground())
